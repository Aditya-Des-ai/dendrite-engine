...
class DendriteControlPanel(tk.Tk):
    ...
    def _build_ui(self):
        # Fixed header
        hdr = tk.Frame(self, bg=BG_DEEP, padx=20, pady=12)
        ...
        self._sf = tk.Frame(self._canvas, bg=BG_DEEP, padx=20)
    ...
    def _build_header(self, p):
        ...
        tk.Label(row, text="DENDRITE ENGINE", bg=BG_DEEP, fg=ACCENT_BLUE,
                 font=("Courier New", 18, "bold"), anchor="w").pack(side=tk.LEFT)
        tk.Label(row, text="SIMULATION CONTROL", bg=BG_DEEP, fg=TEXT_DIM,
                 font=("Courier New", 12)).pack(side=tk.RIGHT)
        tk.Frame(p, bg=ACCENT_BLUE, height=1).pack(fill=tk.X, pady=(10, 0))

    # ── Section divider ───────────────────────────────────────────────────────

    def _sec(self, p, text):
        tk.Label(p, text=text, bg=BG_DEEP, fg=TEXT_DIM,
                 font=("Courier New", 12, "bold"), anchor="w").pack(
                     fill=tk.X, pady=(14, 3))
    ...
    def _make_slider(self, p, key, label, mn, mx, default, unit, desc):
        f = tk.Frame(p, bg=BG_PANEL, padx=14, pady=8)
        f.pack(fill=tk.X, pady=2)
        self._bdr(f)

        # Top row: label on left, value + unit on right
        top = tk.Frame(f, bg=BG_PANEL)
        top.pack(fill=tk.X)
        tk.Label(top, text=label.upper(), bg=BG_PANEL, fg=TEXT_PRI,
                 font=("Courier New", 13, "bold"), anchor="w").pack(side=tk.LEFT)

        var = tk.DoubleVar(value=default)
        self._slider_vars[key] = var

        ro = tk.Label(top, text=self._fmt(key, default), bg=BG_PANEL,
                      fg=ACCENT_BLUE, font=("Courier New", 17, "bold"),
                      anchor="e")
        ...
        if unit:
            tk.Label(top, text=unit, bg=BG_PANEL, fg=TEXT_DIM,
                     font=("Courier New", 10)).pack(side=tk.RIGHT, padx=(0, 6))

        # Slider — compact, no wasted space
        ...
        tk.Scale(f, from_=mn, to=mx, orient=tk.HORIZONTAL,
                 ...
                 highlightthickness=0, bd=0, sliderlength=18, sliderrelief=tk.FLAT,
        ...
        tk.Label(f, text=desc, bg=BG_PANEL, fg=TEXT_DIM,
                 font=("Courier New", 10), anchor="w").pack(fill=tk.X)
    ...
    def _build_charge_toggle(self, p):
        ...
        self._fast_btn = tk.Button(
            ...
            relief=tk.FLAT, bd=0, font=("Courier New", 12),
    ...
    def _build_sands_panel(self, p):
        ...
        f = tk.Frame(p, bg=BG_PANEL, padx=14, pady=12)
        ...
        self._sand_tau.pack(side=tk.LEFT, padx=(8, 0))
        ...
        self._sand_lbl = tk.Label(r2, text=" SAFE — rate below branching limit",
                                  bg=BG_PANEL, fg=SAND_SAFE, font=("Courier New", 12))
        self._sand_lbl.pack(side=tk.LEFT, padx=(6, 0))
    ...
    def _build_buttons(self, p):
        ...
        self._run_btn = tk.Button(
            ...
            relief=tk.FLAT, bd=0, font=("Courier New", 13, "bold"),
    ...
    def _build_status_bar(self):
        bar = tk.Frame(self, bg=BG_PANEL, pady=6, padx=14)
